...
Complete guide for integrating autolemetry with Datadog using OpenTelemetry Protocol (OTLP).
...
Autolemetry provides first-class support for Datadog through the industry-standard **OpenTelemetry Protocol (OTLP)**. This approach offers several advantages over vendor-specific integrations:
...
When you integrate autolemetry with Datadog, you get:
...
If you're currently using `pino-datadog-transport`, autolemetry provides a superior alternative with traces and automatic correlation.
...
- ✅ Only requires installing `autolemetry`

#### Strategy 2: Full Migration

Replace everything with autolemetry OTLP.
...
For custom sampling:
```typescript
import { AdaptiveSampler } from 'autolemetry/sampling';
...
**Solution**: Use `autolemetry-edge` (not regular `autolemetry`)

```typescript
import { init, trace } from 'autolemetry-edge';
...
init({
  service: 'my-app',
  logger, // Autolemetry will log export errors
  // ...
});
```
...
**Solution**: Ensure you're using `createLogger()` from autolemetry:
...
### Q: Can I use both pino-datadog-transport and autolemetry?

**A**: Yes! Use pino-datadog-transport for logs and autolemetry for traces/metrics. This is the recommended incremental migration strategy. See [Migration Strategy 1](#strategy-1-incremental-recommended).
...
### Q: What's the difference between autolemetry and @datadog/dd-trace?

**A**:
- `@datadog/dd-trace`: Datadog-specific tracing library, proprietary protocol
- `autolemetry`: Vendor-neutral OpenTelemetry wrapper, OTLP standard

Autolemetry is better if you want vendor flexibility. Both work with Datadog.
...
2. Use autolemetry for traces/metrics only
...
**A**: Use `autolemetry-edge` instead of `autolemetry`. Same API, optimized for edge runtimes.
...
- [Autolemetry GitHub](https://github.com/jagreehal/autolemetry)

---

## Support

**Autolemetry Issues**: [GitHub Issues](https://github.com/jagreehal/autolemetry/issues)
