# Migration Guide: From OpenTelemetry to Autolemetry

This guide shows how to migrate from vanilla OpenTelemetry to autolemetry.
...
> **Heads up**: Autolemetry lazily loads `@opentelemetry/auto-instrumentations-node`
...
### Step 3: Replace With Autolemetry
...
- [Migration Guide: From OpenTelemetry to Autolemetry](#migration-guide-from-opentelemetry-to-autolemetry)
  - [Quick Migration](#quick-migration)
    - [Step 1: Install](#step-1-install)
    - [Step 2: Find Your Current OpenTelemetry Setup](#step-2-find-your-current-opentelemetry-setup)
    - [Step 3: Replace With Autolemetry](#step-3-replace-with-autolemetry)
...
| OpenTelemetry Pattern                                                         | Autolemetry Equivalent                   | What Changes                        |
| ----------------------------------------------------------------------------- | ---------------------------------------- | ----------------------------------- |
...
| Manual log correlation                                                        | `autolemetry/logger`                     | Automatic trace context injection   |
...
// Autolemetry auto-detects the pattern and handles lifecycle
...
```typescript
import { AdaptiveSampler, init } from 'autolemetry';
...
```typescript
import { init, type Sampler, type SamplingContext } from 'autolemetry';
...
```typescript
import { createCounter } from 'autolemetry/metrics';
...
| Manual log correlation              | `autolemetry/logger`    | Built-in correlation        |
...
Your collector config **doesn't need to change**. Autolemetry uses standard OTLP protocol.
...
- [ ] Use `createLogger()` from `autolemetry/logger`
...
- **Issues**: [GitHub Issues](https://github.com/jagreehal/autolemetry/issues)
