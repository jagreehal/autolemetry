# AI/LLM Workflow Patterns with Autolemetry

This guide demonstrates how to instrument AI/LLM applications using Autolemetry's existing APIs. All patterns shown leverage standard OpenTelemetry primitives and work seamlessly with OpenLLMetry's automatic LLM instrumentation.
...
Autolemetry provides all the building blocks needed for comprehensive AI/LLM observability:
...
```typescript
import { trace, track, init } from 'autolemetry';
...
**Key Takeaway**: Autolemetry's existing functional API (`trace()`, `ctx`, `track()`) provides everything needed for comprehensive AI/LLM observability. Combined with OpenLLMetry's automatic instrumentation, you get:
