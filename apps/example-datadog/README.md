# Datadog + Autolemetry Integration Example

This example demonstrates how to send **all observability signals** (traces, logs, and metrics) from Node.js to Datadog using autolemetry and the OpenTelemetry Protocol (OTLP).

## Overview

This example shows how autolemetry provides a unified, vendor-neutral approach to observability with Datadog:
...
> **Note**: This example installs `@opentelemetry/sdk-logs` and `@opentelemetry/exporter-logs-otlp-http` because they are **optional peer dependencies** of autolemetry. They are only needed for log export via OTLP. If you only want **traces and metrics** (not logs), you can omit these packages and remove the `logRecordProcessors` configuration from `init()`.
...
The example will:
1. Initialize autolemetry with Datadog OTLP endpoints
...
Autolemetry supports **two architectures** for sending telemetry to Datadog. Choose the approach that best fits your deployment:
...
Change your autolemetry config to point to the local Agent:
...
See `/packages/autolemetry/src/presets/datadog.ts` for full API.
...
```typescript
import { recordMetric } from 'autolemetry';
...
If you're currently using `pino-datadog-transport`, this example shows how to replace it with autolemetry's OTLP-based approach.
...
| Feature | pino-datadog-transport | Autolemetry OTLP |
|---------|------------------------|------------------|
...
| **Additional Dependencies** | pino-datadog-transport | Only autolemetry<br/>(+optional log packages if needed) |
| **Unified Observability** | ❌ Logs only | ✅ All signals |

### Incremental Migration Strategy

**Option 1: Hybrid Approach (Recommended for existing apps)**
Keep `pino-datadog-transport` for logs, add autolemetry for traces and metrics:
...
// Add autolemetry for traces and metrics only
import { init } from 'autolemetry';
...
- ✅ Low risk migration
- ✅ Only need to install `autolemetry` (no log packages)

**Option 2: Full Migration**
Replace everything with autolemetry OTLP (this example app):
- Install log packages: `@opentelemetry/sdk-logs`, `@opentelemetry/exporter-logs-otlp-http`
- Configure `logRecordProcessors` in `init()`
- Use `createLogger()` from autolemetry
- Remove `pino-datadog-transport`

### Key Differences

1. **Protocol**: pino-datadog-transport uses Datadog's proprietary log intake API; autolemetry uses standard OTLP
2. **Signals**: pino-datadog-transport only handles logs; autolemetry provides traces, logs, and metrics
3. **Correlation**: pino-datadog-transport requires manual trace ID injection; autolemetry does this automatically
...
1. Ensure you're using `createLogger()` from autolemetry, not raw Pino
...
```typescript
import { AdaptiveSampler } from 'autolemetry';
...
- [Autolemetry Documentation](https://github.com/jagreehal/autolemetry)
...
- Autolemetry: [GitHub Issues](https://github.com/jagreehal/autolemetry/issues)
