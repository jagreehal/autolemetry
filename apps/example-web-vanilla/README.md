# Autolemetry Web - Vanilla JavaScript Example

Ultra-lightweight example demonstrating browser-to-backend distributed tracing with autolemetry-web.
...
1. Build the autolemetry-web package:
```bash
...
```typescript
import { init, trace } from 'autolemetry-web'
...
The backend using Autolemetry automatically extracts the `traceparent` header:
...
The autolemetry-web SDK loaded by this example is only **1.6KB gzipped**!

Check the bundle in DevTools:
1. Network tab → JS filter
2. Look for `index.js` from autolemetry-web
3. Size column shows: ~5KB uncompressed, ~1.6KB gzipped

## Next Steps

1. **Try with your own API:** Replace the mock API URL with your backend
2. **Add Autolemetry to your backend:** See [Autolemetry docs](../../packages/autolemetry)
...
1. Check the console for initialization message:
   ```
   [autolemetry-web] Initialized successfully
   ```
...
- [autolemetry-web Documentation](../../packages/autolemetry-web/README.md)
- [Autolemetry (Backend) Documentation](../../packages/autolemetry/README.md)
