# Postgres + Autolemetry Example

This example shows how to pair the official [`@opentelemetry/instrumentation-pg`](https://github.com/open-telemetry/opentelemetry-js-contrib/tree/main/plugins/node/instrumentation-pg) package with [autolemetry](../../packages/autolemetry/README.md) to capture spans for Postgres clients that use the widely adopted `pg` driver.
...
`src/instrumentation.mjs` initializes autolemetry and the official pg instrumentation before anything else:
...
await client.query('SELECT $1::text as message', ['Hello from autolemetry!']);
```
...
After the sample queries, the app calls `shutdown()` from autolemetry to flush spans before exiting, ensuring your OTLP exporter receives the captured data.
...
- Set env vars such as `OTEL_SERVICE_NAME`, `OTEL_EXPORTER_OTLP_ENDPOINT`, and exporter headers instead of hardcoding them. Autolemetry automatically reads these values.
...
- [autolemetry documentation](../../packages/autolemetry/README.md)
