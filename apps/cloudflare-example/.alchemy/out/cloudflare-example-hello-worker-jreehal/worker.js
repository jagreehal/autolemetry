...
// ../../node_modules/.pnpm/alchemy@0.77.5_@libsql+client@0.15.15_drizzle-orm@0.44.7_@cloudflare+workers-types@4.20_cf739edc2ab6831528a9153b0b90b875/node_modules/alchemy/lib/cloudflare/bundle/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
...
// ../../node_modules/.pnpm/alchemy@0.77.5_@libsql+client@0.15.15_drizzle-orm@0.44.7_@cloudflare+workers-types@4.20_cf739edc2ab6831528a9153b0b90b875/node_modules/alchemy/lib/cloudflare/bundle/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
var require_context_utils = __commonJS({
  "../../node_modules/.pnpm/@opentelemetry+api@1.9.0/node_modules/@opentelemetry/api/build/src/trace/context-utils.js"(exports) {
    ...
    function setSpan(context4, span2) {
      return context4.setValue(SPAN_KEY, span2);
    }
...
// ../../packages/autolemetry-edge/dist/chunk-27S4NVRT.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
var ResourceImpl = class _ResourceImpl {
  ...
  get attributes() {
    ...
    for (const [k, v] of this._rawAttributes) {
      if (isPromiseLike(v)) {
        import_api.diag.debug(`Unsettled resource attribute ${k} skipped`);
        continue;
      }
      if (v != null) {
        attrs[k] ??= v;
  ...
  return attributes.map(([k, v]) => {
    if (isPromiseLike(v)) {
      return [
        k,
        v.catch((err) => {
          import_api.diag.debug("promise rejection for resource attribute: %s - %s", k, err);
          return void 0;
        })
      ];
    }
...
function validateSchemaUrl(schemaUrl) {
  ...
  import_api.diag.warn("Schema URL must be string or undefined, got %s. Schema URL will be ignored.", schemaUrl);
  return void 0;
}
...
function mergeSchemaUrl(old, updating) {
  ...
  import_api.diag.warn('Schema URL merge conflict: old resource has "%s", updating resource has "%s". Resulting resource will have undefined Schema URL.', oldSchemaUrl, updatingSchemaUrl);
  return void 0;
}
...
var ParentBasedSampler = class {
  ...
  shouldSample(context4, traceId, spanName, spanKind, attributes, links) {
    const parentContext = import_api2.trace.getSpanContext(context4);
    if (!parentContext || !(0, import_api2.isSpanContextValid)(parentContext)) {
  ...
  async exportSpans(spans) {
    ...
    if (this.postProcessor) {
      try {
        ...
        console.error("[autolemetry-edge] Post-processor error:", error3);
...
var TailSamplingSpanProcessor = class {
  ...
  onEnd(span2) {
    ...
    const trace33 = this.traces.get(traceId);
    const hasLocalParent = parentSpanId && trace33.spans.some((s) => s.spanContext().spanId === parentSpanId);
    if (!hasLocalParent) {
      trace33.localRootSpan = span2;
    }
    trace33.spans.push(span2);
    const isDefinitiveRoot = !parentSpanId;
    const shouldAutoFlush = isDefinitiveRoot && trace33.localRootSpan && trace33.localRootSpan.spanContext().spanId === spanId;
    if (shouldAutoFlush) {
      if (this.tailSampler) {
        const shouldKeep = this.tailSampler(trace33);
        if (shouldKeep) {
          for (const bufferedSpan of trace33.spans) {
  ...
  async forceFlush(traceId) {
    if (traceId) {
      const trace33 = this.traces.get(traceId);
      if (trace33) {
        if (!trace33.localRootSpan && trace33.spans.length > 0) {
          trace33.localRootSpan = trace33.spans[0];
        }
        if (this.tailSampler) {
          const shouldKeep = this.tailSampler(trace33);
          if (shouldKeep) {
...
var CONFIG_KEY = (0, import_api3.createContextKey)("autolemetry-edge-config");
function getActiveConfig() {
  const value = import_api3.context.active().getValue(CONFIG_KEY);
  return value ?? null;
}
__name(getActiveConfig, "getActiveConfig");
function setConfig(config2) {
  return import_api3.context.active().setValue(CONFIG_KEY, config2);
}
...
function instrumentGlobalFetch() {
  ...
  const instrumentedFetch = /* @__PURE__ */ __name(function fetch2(input, init) {
    ...
    const tracer = import_api3.trace.getTracer("autolemetry-edge");
    const url = new URL(request.url);
      ...
      async (span2) => {
        try {
          ...
          if (shouldIncludeContext) {
            import_api3.propagation.inject(import_api3.context.active(), request.headers, {
          ...
          if (response.ok) {
            ...
            span2.setStatus({ code: import_api3.SpanStatusCode.ERROR });
          }
          return response;
        } catch (error3) {
...
var WorkerTracer = class {
   ...
   * Start a new span
   */
  startSpan(name, options = {}, context22 = import_api4.context.active()) {
    if (options.root) {
      context22 = import_api4.trace.deleteSpan(context22);
    }
    if (!this.headSampler) {
      throw new Error(
    ...
    const parentSpanContext = import_api4.trace.getSpan(context22)?.spanContext();
  ...
  startActiveSpan(name, ...args) {
    ...
    const parentContext = args.length > 2 ? args[1] : import_api4.context.active();
    const fn = args[args.length - 1];
    const span2 = this.startSpan(name, options, parentContext);
    const contextWithSpanSet = import_api4.trace.setSpan(parentContext, span2);
    return import_api4.context.with(contextWithSpanSet, fn, void 0, span2);
  }
};
function getTraceInfo(parentSpanContext) {
  if (parentSpanContext && import_api4.trace.isSpanContextValid(parentSpanContext)) {
...
var WorkerTracerProvider = class {
  ...
  register() {
    ...
    import_api4.trace.setGlobalTracerProvider(provider);
  }
};
...
function wrapWithTracingAsync(fnFactory, options, variableName) {
  ...
  const wrappedFunction = /* @__PURE__ */ __name(async function wrappedFunction2(...args) {
    const tracer = import_api4.trace.getTracer("autolemetry-edge");
    ...
    return tracer.startActiveSpan(spanName, spanOptions, async (span2) => {
      ...
      try {
        ...
        span2.setAttribute("code.function", spanName);
        span2.setStatus({ code: import_api4.SpanStatusCode.ERROR, message });
  ...
  return {
    getInitialSpanInfo: /* @__PURE__ */ __name((request) => {
      ...
      return {
        ...
        context: import_api4.propagation.extract(import_api4.context.active(), request.headers)
      };
    }, "getInitialSpanInfo"),
...
function addQueueEvent(name, msg, delaySeconds) {
  ...
  import_api4.trace.getActiveSpan()?.addEvent(name, attrs);
}
        ...
        return new Proxy(retryFn, {
          apply: /* @__PURE__ */ __name((fnTarget, _thisArg, args) => {
            ...
            if (retryOptions?.contentType) {
              const span2 = import_api4.trace.getActiveSpan();
              if (span2) {
...
function createHandlerFlow(instrumentation) {
  return (handlerFn, [trigger, env2, context22]) => {
    const { ctx: proxiedCtx, tracker } = proxyExecutionContext(context22);
    const tracer = import_api4.trace.getTracer("autolemetry-edge");
    ...
    coldStart = false;
    const parentContext = spanContext || import_api4.context.active();
    ...
    return tracer.startActiveSpan(name, options, parentContext, async (span2) => {
      try {
        ...
        span2.end();
        context22.waitUntil(exportSpans(span2.spanContext().traceId, tracker));
      }
    });
  };
}
...
function createHandlerProxy(_handler, handlerFn, initialiser, instrumentation) {
  return (trigger, env2, ctx) => {
    ...
    return import_api4.context.with(configContext, () => {
...
function initProvider(config2) {
  ...
  import_api4.propagation.setGlobalPropagator(config2.propagator);
...
function instrument2(handler2, config2) {
...
__name(instrument2, "instrument2");
...
function handleError(error3, event, options) {
  ...
  console.error("[autolemetry-edge] Adapters transport failed", error3, { event });
}
...
function createAdaptersInstance(options, bindings = {}) {
  ...
  return {
    ...
    trackValue,
    dispatch,
    bind: /* @__PURE__ */ __name((nextBindings) => createAdaptersInstance(options, { ...bindings, ...nextBindings }), "bind")
  };
}
__name(createAdaptersInstance, "createAdaptersInstance");
function createEdgeAdapters(options) {
  if (typeof options.transport !== "function") {
    throw new Error("createEdgeAdapters: options.transport is required");
  }
  return createAdaptersInstance(options);
}
__name(createEdgeAdapters, "createEdgeAdapters");
function getEdgeAdapters(ctx) {
  const config2 = getActiveConfig();
  if (!config2) {
    return null;
  }
  const adapters = config2.adapters ?? [];
  if (!adapters.length) {
    return null;
  }
    ...
    await Promise.all(
      adapters.map(async (adapter, index) => {
        try {
          ...
          console.error("[autolemetry-edge] Adapters adapter failed", error3, {
            adapterIndex: index,
...
__name(getEdgeAdapters, "getEdgeAdapters");
function getTraceContext() {
  ...
  return {
    traceId: ctx.traceId,
    spanId: ctx.spanId,
    correlationId: ctx.traceId.substring(0, 16)
...
function createEdgeLogger(service, options) {
  ...
  const log4 = /* @__PURE__ */ __name((level, msg, attrs) => {
    ...
    if (pretty) {
      const traceInfo = ctx ? ` [${ctx.traceId.substring(0, 8)}.../${ctx.spanId.substring(0, 8)}...]` : "";
      console.log(
...
// ../../packages/autolemetry-edge/dist/sampling/index.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
var worker_default = instrument2(handler, (env2) => ({
  exporter: {
