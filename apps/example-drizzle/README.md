# OpenTelemetry tracing with Autolemetry for Drizzle ORM

Autolemetry provides a simplified approach to setting up OpenTelemetry tracing for Drizzle ORM. This page shows you how to use autolemetry to reduce OpenTelemetry setup code while maintaining full control over instrumentation.

> **Note**: Autolemetry is a third-party package that wraps the standard OpenTelemetry SDK. It simplifies the setup process and produces the same trace output as manual OpenTelemetry setup.
...
Autolemetry reduces this setup to a single `init()` function call while maintaining compatibility with the OpenTelemetry standard. You still get the same powerful tracing capabilities, but with less configuration code.

## Considerations and prerequisites

To use autolemetry with Drizzle, you must:

- Install the `autolemetry` package
- Install the `autolemetry-plugins` package
- Install `drizzle-orm` and the appropriate database driver (e.g., `pg`, `mysql2`, `@libsql/client`)
- Initialize autolemetry before creating your Drizzle client

> **Important**: The order in which you set up tracing matters. You must initialize autolemetry before creating the Drizzle client instance.

## Get started with autolemetry

This section explains how to install and configure autolemetry for Drizzle tracing.
...
```bash
npm install autolemetry autolemetry-plugins drizzle-orm pg
npm install -D @types/pg
```

### Step 2: Initialize autolemetry and instrument the client

Initialize autolemetry and then use `instrumentDrizzleClient` to wrap your Drizzle instance:
...
## What autolemetry configures

When you call `init()`, autolemetry automatically configures:

| Component | Manual Setup | Autolemetry Configuration |
|-----------|--------------|---------------------------|
...
## Comparison: Manual setup vs. autolemetry

The following examples show the difference in setup code between manual OpenTelemetry configuration and autolemetry.
...
The same configuration using autolemetry:
...
Autolemetry produces detailed trace output for Drizzle operations. For each query, a span is created with:
...
Autolemetry includes a `trace()` function for wrapping your application logic with tracing:
...
Autolemetry uses tail sampling by default, which makes sampling decisions after operations complete. This ensures you capture 100% of errors and slow requests while sampling only a percentage of successful operations.

To customize sampling behavior:

```typescript
import { init, AdaptiveSampler } from 'autolemetry';
...
## When to use autolemetry vs. manual setup

Choose **autolemetry** if:
...
Using autolemetry doesn't change:
...
Autolemetry is a convenience wrapper that reduces boilerplate while maintaining full OpenTelemetry compatibility.
...
- [Full autolemetry documentation](https://github.com/jagreehal/autolemetry)
- [Migration guide from manual OpenTelemetry setup](https://github.com/jagreehal/autolemetry/blob/main/docs/MIGRATION.md)
- [Working Drizzle + autolemetry example](https://github.com/jagreehal/autolemetry/tree/main/apps/example-drizzle)
