...
export const FEATURE_FLAGS = {
  ...
  ENABLE_AUTO_INSTRUMENTATION:
    IS_PRODUCTION && process.env.autolemetry_AUTO_INSTRUMENT !== 'false',

  /** Enable verbose logging (development only) */
  ENABLE_VERBOSE_LOGGING: IS_DEV || process.env.autolemetry_VERBOSE === 'true',

  /** Enable metrics collection (production only) */
  ENABLE_METRICS_BY_DEFAULT:
    IS_PRODUCTION && process.env.autolemetry_METRICS !== 'false',

  /** Enable async resource detection (production only) */
  ENABLE_RESOURCE_DETECTION:
    IS_PRODUCTION && process.env.autolemetry_RESOURCE_DETECTION === 'true',

  /** Enable tracing in all environments (can be disabled via autolemetry_TRACING=false) */
  ENABLE_TRACING: process.env.autolemetry_TRACING !== 'false',

  /** Enable log redaction for sensitive fields (can be disabled via autolemetry_REDACTION=false) */
  ENABLE_REDACTION: process.env.autolemetry_REDACTION !== 'false',
} as const;
 ...
 * @example
 * ```typescript
 * import { configure } from 'autolemetry/config'
 *
 * configure({
 *   tracerName: 'my-app'
 * })
