/**
 * Functional API for autolemetry-edge
 *
...
const TRACE_FACTORY_SYMBOL = Symbol.for('autolemetry.edge.functional.factory');
...
const INSTRUMENTED_SYMBOL = Symbol.for('autolemetry.edge.functional.instrumented');
...
function wrapWithTracingAsync<TArgs extends any[], TReturn>(
  ...
  const wrappedFunction = async function wrappedFunction(this: unknown, ...args: TArgs): Promise<TReturn> {
    const tracer = otelTrace.getTracer('autolemetry-edge');
