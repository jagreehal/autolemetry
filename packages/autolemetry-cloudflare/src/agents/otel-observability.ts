 ...
 * import { createOtelObservability } from 'autolemetry-cloudflare/agents'
 *
...
function initProvider(config: ResolvedEdgeConfig): void {
  ...
  const resource = resourceFromAttributes({
    ...
    'telemetry.sdk.name': 'autolemetry-cloudflare',
  ...
  const tracer = trace.getTracer('autolemetry-cloudflare/agents') as WorkerTracer;
...
async function exportSpans(traceId: string, ctx?: DurableObjectState): Promise<void> {
  const tracer = trace.getTracer('autolemetry-cloudflare/agents');
  if (tracer instanceof WorkerTracer) {
    try {
      await scheduler.wait(1);
      await tracer.forceFlush(traceId);
    } catch (error) {
      console.error('[autolemetry-cloudflare/agents] Failed to export spans:', error);
    }
  }
